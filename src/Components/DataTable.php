...
class DataTable extends UI
{
     ...
     * @param string $widget_icon
     */
    public function __construct($data, $options = [], $widget_title = '<h2>DataTable Result Set</h2>', $widget_icon = 'table')
    {
    ...
    private function _init_structure($data, $user_options, $widget_title, $widget_icon)
    {
        ...
        if (!$this->_structure->data) {
            $col_list = $this->_structure->options["default_col"] ? array($this->_structure->options["default_col"]) : [];
        } else {
            $col_list = array_keys(is_object($data[0]) ? get_object_vars($data[0]) : $data[0]);
        }

        $cols = array_combine($col_list, $col_list);
        $cells = array_fill_keys($col_list, []);
        ...
        $id = Util::get_property_value(
            ...
            array(
                "if_closure" => function ($prop) {
                    return Util::run_callback($prop, array($this));
                },
